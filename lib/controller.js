...
// // purely for mock testing purposes
  ...
  streamVideo(stream, callObj, localStream = null, doc=document) {
    ...
    // Display remote person's video side-by-side
    if (stream) {
  ...
  showRemoteVideoPIP(remoteStream, peerName, doc=document) {
    // Create or get remote video element for side-by-side display
    ...
    if (!remoteVid) {
      ...
      // Create a container for side-by-side layout if it doesn't exist
      ...
      if (!videoContainer) {
        ...
        if (mainVid) {
          ...
          modal.insertBefore(videoContainer, mainVid);
